import time
from agents import (
    build_reader_agent,
    build_search_agent,
    writer_chain,
    critic_chain,
    revision_chain,
)


def run_research_pipeline(topic: str) -> dict:
    state = {}

    # Step 1: Search Agent
    print("\n" + "=" * 50)
    print("Step 1 - Search Agent is working...")
    print("=" * 50)

    search_agent = build_search_agent()

    search_result = search_agent.invoke({
        "messages": [
            (
                "user",
                f"Find recent, reliable and detailed information about: {topic}"
            )
        ]
    })

    state["search_results"] = search_result["messages"][-1].content

    print("\nSearch Results:\n", state["search_results"])
    print("\nWaiting for Gemini rate limit...")
    time.sleep(65)

    # Step 2: Reader Agent
    print("\n" + "=" * 50)
    print("Step 2 - Reader Agent is scraping resources...")
    print("=" * 50)

    reader_agent = build_reader_agent()

    reader_result = reader_agent.invoke({
        "messages": [
            (
                "user",
                f"""
                Based on the following search results about '{topic}',
                select the three most relevant URLs and scrape them.

                Search Results:
                {state["search_results"]}
                """
            )
        ]
    })

    state["scraped_content"] = reader_result["messages"][-1].content

    print("\nScraped Content:\n", state["scraped_content"])
    print("\nWaiting for Gemini rate limit...")
    time.sleep(65)

    # Combine research
    research_combined = (
        f"SEARCH RESULTS:\n{state['search_results']}\n\n"
        f"DETAILED SCRAPED CONTENT:\n{state['scraped_content']}"
    )

    # Step 3: Writer Chain
    print("\n" + "=" * 50)
    print("Step 3 - Writer is drafting the report...")
    print("=" * 50)

    state["report"] = writer_chain.invoke({
        "topic": topic,
        "research": research_combined,
    })

    print("\nInitial Report:\n", state["report"])

    # Step 4: Critic Chain
    print("\n" + "=" * 50)
    print("Step 4 - Critic is reviewing the report...")
    print("=" * 50)

    state["feedback"] = critic_chain.invoke({
        "report": state["report"]
    })

    print("\nCritic Feedback:\n", state["feedback"])

    # Step 5: Revision Chain
    print("\n" + "=" * 50)
    print("Step 5 - Writer is improving the report...")
    print("=" * 50)

    state["final_report"] = revision_chain.invoke({
        "topic": topic,
        "research": research_combined,
        "report": state["report"],
        "feedback": state["feedback"],
    })

    print("\nImproved Final Report:\n", state["final_report"])

    # Return only after every step is completed
    return state


if __name__ == "__main__":
    topic = input("\nEnter a research topic: ")
    run_research_pipeline(topic)